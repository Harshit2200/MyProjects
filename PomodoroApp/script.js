...
const checkTimeEnd = (minVal, secVal) => {
  if (minVal == 0 && secVal === 0) {
    ...
    document
      .querySelector(".timer-wrapper-middle:after")
      .style.setProperty("color", "red");
  }
};

const resetTime = () => {
  min.innerHTML = document
    .querySelector(".time-option.active")
...
const setProgress = (per) => {
  document
    .querySelector(".timer-wrapper-middle")
    .style.setProperty("--progress", per + "%");
};
...
const applySettings = () => {
  ...
  const font = document
    .querySelector(".font-option.active")
  ...
  if (
    Math.trunc(pomoDoroLength) != pomoDoroLength ||
    Math.trunc(shortBreakLength) != shortBreakLength ||
    Math.trunc(longBreakLength) != longBreakLength
  ) {
    alert("Please enter an integer value");
    return;
  }
  document
    .querySelector("#pomodoro")
    .setAttribute("data-time", "" + pomoDoroLength);
  document
    .querySelector("#short")
    .setAttribute("data-time", "" + shortBreakLength);
  document
    .querySelector("#long")
    .setAttribute("data-time", "" + longBreakLength);
  resetTime();
  dialog.close();
};
